...
func TestUsagePrints(t *testing.T) {
	...
	t.Run("printSandboxUsage outputs sandbox usage", func(t *testing.T) {
		...
		assert.Contains(t, output, "sandbox <new|validate|list|show|update|start|stop|pause|resume|revert|snapshot|destroy|lease|prune|expose|exposed|unexpose|doctor>")
	})
...
func TestGoldenFileSandboxUsageOutput(t *testing.T) {
	...
	assert.Contains(t, got, "agentlab sandbox <new|validate|list|show|update|start|stop|pause|resume|revert|snapshot|destroy|lease|prune|expose|exposed|unexpose|doctor>")
}
