...
func TestUsagePrints(t *testing.T) {
	...
	t.Run("printSandboxUsage outputs sandbox usage", func(t *testing.T) {
		...
		assert.Contains(t, output, "sandbox <new|list|show|start|stop|pause|resume|revert|destroy|lease|prune|expose|exposed|unexpose|doctor>")
	})
...
func TestGoldenFileSandboxUsageOutput(t *testing.T) {
	...
	assert.Contains(t, got, "agentlab sandbox <new|list|show|start|stop|pause|resume|revert|destroy|lease|prune|expose|exposed|unexpose|doctor>")
}
